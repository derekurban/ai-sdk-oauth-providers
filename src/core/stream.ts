...
import { toFinishReason, toResponseMetadata, toUsage } from "./result.js";
  ...
  return new ReadableStream<LanguageModelV3StreamPart>({
    async start(controller) {
      ...
      for await (const event of source) {
        for (const part of mapEventToStreamParts(event)) {
...
function mapEventToStreamParts(event: RuntimeStreamEvent): LanguageModelV3StreamPart[] {
  switch (event.type) {
    case "start":
